...
[start=1]
. The WebService call to the Flight Service will execute in parallel with the call to the Hotel Service wis has already been defined in our process model. We therefore first need to add a diverging and converging parallel  gateway to our process.
...
image:images/lab8-start-validating.png["Validate the process", 256]
